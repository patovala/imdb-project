...
\emph{Authors:}\\
Hammad Javed\\
Iv\'an Patricio \textsc{Valarezo}\\ % Your name
Saman Bonab\\
\end{flushleft}
\end{minipage}
~
...
The MMDB presents a new challenge or oportunity (depends of the point of view). Taking in account the new hardware, the expected continuity of the Moore's law, and the relatively low prices of main memory, we could expect to see a grown toward MMDB systems. Although, there still are some criticism to MMDB in terms of becoming the link and dreamed merge of OLTP and OLAP systems \cite{putwhitepaper}.

Of course, in the embeded systems field, we are optimistics about the MMDB role to the sucess of small and powerfull devices and the MMDB technology as a key point and best choice to go.
